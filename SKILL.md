...
**绝对红线**：当老板提供一批新物品进行盘点时，**严禁直接修改数据文件或执行同步脚本**。必须严格遵循以下交互步骤：
1. **严格查字典**：必须首先对照 `CLASSIFICATION_STANDARD.md`，将物品精确映射到现有的标准大类和子类（绝不能自造分类）。
2. **提议新分类**：若遇到确实无法归入现有分类的特殊物品，必须单独列出并向老板提议新增分类，**必须等待老板明确同意**后才能添加。
3. **输出审核方案**：向老板输出初步分类结果与拟存放的容器位置，请求审核。
4. **等待执行指令**：只有在老板明确回复“确认”、“执行”或“录入”后，方可编写脚本修改 `items.json` 并调用 `sync_final.py` 固化数据。
