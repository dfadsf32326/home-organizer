...
2. **建立临时盘点会话 (Staging)**：将用户报出的物品汇总后暂存（如写入 `/tmp/` 或本地 `data/inventory_staging.json`），**绝对不要向主库 `items.json` 写入，更不要推送到飞书**。
3. **分类展示与确认**：向老板输出初步分类清单，供老板审核。如果老板确认需要新建系统不存在的分类，必须先通过 lark-cli 在飞书中新建分类并获取 `record_id`，同步回本地的 mapping。
4. **落库与增量同步**：在全部分类和 ID 对齐后，将这批数据从暂存区正式移入本地 `items.json`，并触发向飞书的批量推送。
...
2. **创建新空间（若确认不是旧空间）**：
   ...
   - 在本地 `data/space_map.json` 等数据库中写入新的空间记录。
   - 立即执行 `sync_spaces_bidirectional.py` 将新空间推送到飞书，确保其拿到合法对应的 Record ID。
