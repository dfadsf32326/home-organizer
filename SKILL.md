...
5. **等待确认后推送**：**必须等待老板明确确认后**，才能调用 `scripts/push_staging.py` 将暂存区（staging）的数据推送到飞书。**绝不能在未确认前自动推送**。推送成功后该脚本会自动清空暂存区，并触发 `sync_final.py`（纯Pull脚本）从飞书全量拉取最新数据覆盖本地 `items.json`。绝不能将盘点数据直接合并到本地主库中。
...
- **名称保留完整信息**：物品名称必须包含数量和单位，绝不能过度简化。推荐格式：`物品名 数量单位`（如"鹅肉 1袋"、"香肠 7袋"、"黄油 2罐"）。这样即使后续飞书表格中数量字段未填写，名称本身也能传达完整信息。
- **备注记录层级**：当一个空间容器内有多个层级（如冰箱冷冻室第1-5层），无需为每层创建独立空间容器。只需在物品的 `remark`（备注）字段记录具体层级（如"冷冻室第一层"、"冷冻室第二层"），既减少空间树复杂度，又能精准定位物品位置。
